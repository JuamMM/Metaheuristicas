...
Poblacion CRO(PAR estado, int num_datos, int datos_centro, int num_clusters, int poblaciones, int min, int max){
	...
	while(evaluaciones<criterio_parada && !parar){
		...
		while(broadcast.size()>1){
			...
			Poblacion padre2 = estado.devuelvePoblacion(broadcast[p2]);
		...
		vector<int> p_sol = estado.peoresPadres(lambda,datos,restricciones,poblaciones*0.2);
...
Poblacion CRO_BL(PAR estado, int num_datos, int datos_centro, int num_clusters, int poblaciones, int min, int max){
	...
	while(evaluaciones<criterio_parada && !parar){
		...
		Poblacion vacio(num_datos, datos_centro, num_clusters, min, max,restricciones,datos,lambda);
		//Recorremos la población eliminando poblaciones en función de su salud
		vector<int> p_sol = estado.peoresPadres(lambda,datos,restricciones,poblaciones*0.3);
		for(auto it_i=p_sol.begin(); it_i!=p_sol.end();it_i++){
			int i= (*it_i);
			float val_p = 0.5*generaciones;
			int p_muerte = rand() % 100;
...
Poblacion CRO_M(PAR estado, int num_datos, int datos_centro, int num_clusters, int poblaciones, int min, int max){
	int rep_sexual;
	int criterio_parada = 100000, evaluaciones = 0, generaciones = 0;
	vector<int> corales;
	vector<int> indices;
	...
	while(evaluaciones<criterio_parada && !parar){
		...
		vector<int> m_padres = estado.mejoresPadres(lambda,datos,restricciones,poblaciones*0.3);
			while(m_padres.size()>1){
			int p1 = rand() % m_padres.size();
			Poblacion padre1 = estado.devuelvePoblacion(m_padres[p1]);
			m_padres.erase(m_padres.begin()+p1);

			int p2 = rand() % m_padres.size();
			Poblacion padre2 = estado.devuelvePoblacion(m_padres[p2]);
			m_padres.erase(m_padres.begin()+p2);
	...
	int mejor_sol = estado.mejorCromosoma(lambda,datos,restricciones);
	Poblacion pob_mejor = estado.devuelvePoblacion(mejor_sol);
	cout<<"Mejor solucion CRO-M"<<endl;
	int error = pob_mejor.calcularErrorGenerado();
	cout<<pob_mejor.desviacionGeneral()<<"\t";
	cout<<error<<"\t";
	cout<<pob_mejor.devolverValoracion()<<"\t";
...
int main(int argc, char **argv){
	if(argc < 5){
		...
		start_timers();
		CRO_M(problema2, numero_datos, datos_centro, clusters, numero_corales, minimo, maximo);
		cout<<elapsed_time()<<endl;
